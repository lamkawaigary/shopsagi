    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        if (userData.role === 'merchant') {
          ...
          await auth.signOut();
          setError('呢個帳戶唔係商戶帳戶，請用顧客帳戶登入');
          setLoading(false);
        } else if (userData.role === 'driver') {
          await auth.signOut();
          setError('呢個帳戶唔係商戶帳戶，請用司機帳戶登入');
          setLoading(false);
        } else {
          await auth.signOut();
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full">
        <div className="text-center mb-6">
          <Link href="/" className="text-2xl font-bold text-blue-600 flex items-center justify-center gap-2"><Store className="w-7 h-7" />ShopSagi 商戶</Link>
        </div>

        <div className="bg-white rounded-xl shadow-lg p-6 md:p-8">
          <h1 className="text-2xl font-bold text-gray-900 mb-2">商戶登入</h1>
          <p className="text-gray-800 mb-6">管理你嘅店鋪同訂單</p>
        ...
        <div className="text-center mt-6">
          <p className="text-gray-800">
            未有商戶帳戶？<Link href="/register" className="text-blue-600 hover:underline font-medium">申請入駐 →</Link>
          </p>
        </div>
      </div>
    </div>
